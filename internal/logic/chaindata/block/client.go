...
func (ec *Client) BlockByHash(ctx context.Context, hash common.Hash) (*types.Block, *common.Hash, []*common.Address, []*common.Hash, error) {
...
// if you don't need all transactions or uncle headers.
func (ec *Client) BlockByNumber(ctx context.Context, number *big.Int) (*types.Block, *common.Hash, []*common.Address, []*common.Hash, error) {
...
func (ec *Client) getBlock(ctx context.Context, method string, args ...interface{}) (*types.Block, *common.Hash, []*common.Address, []*common.Hash, error) {
	...
	if len(body.UncleHashes) > 0 {
		...
		for i := range reqs {
			if reqs[i].Error != nil {
				return nil, nil, nil, nil, reqs[i].Error
			}
			if uncles[i] == nil {
				return nil, nil, nil, nil, fmt.Errorf("got null header for uncle %d of block %x", i, body.Hash[:])
			}
		}
	}
	// Fill the sender cache of transactions in the block.
	txs := make([]*types.Transaction, len(body.Transactions))
	txFroms := make([]*common.Address, len(body.Transactions))
