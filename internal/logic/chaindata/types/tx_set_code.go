...
// func bigToU256(b *big.Int) hexutil.U256 {
// 	var u hexutil.U256
// 	// 将 big.Int 转换为字节数组
// 	bytes := b.Bytes()
...
// 	// 确保字节长度不超过 32 字节
// 	if len(bytes) > 32 {
// 		panic("big.Int exceeds 256 bits")
// 	}

// 	// 将字节复制到 u256 的字节数组中，从高位开始填充
// 	(*uint256.Int)(&u).SetBytes(padLeft(bytes))
// 	return u
// }
// func padLeft(bytes []byte) []byte {
// 	length := len(bytes)
// 	if length >= 32 {
// 		return bytes[:32] // 截断超过32字节的部分（根据需要可以处理错误）
// 	}
// 	padded := make([]byte, 32)
// 	copy(padded[32-length:], bytes)
// 	return padded
// }
...
func (a *SetCodeAuthorization) Authority() (common.Address, error) {
	...
	r, ok := uint256.FromBig(a.R.ToInt())
	if !ok {
		return common.Address{}, ErrInvalidSig
	}
	s, ok := uint256.FromBig(a.S.ToInt())
	if !ok {
		return common.Address{}, ErrInvalidSig
	}
